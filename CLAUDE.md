...
- Everything is config-driven JSON in `configs/`, one file per stage (`annotator_config.json` holds the large boilerplate-stripping substring lists, `clusterer_config.json` the distance penalties, `ranker_config.json` the per-issue thresholds, `daemon_config.json` the loop timings). Behavior changes usually belong there, not in code.
